...
function build_insert_query($table, $data) { // Insert Query Builder Function
    $keys = array();
    $values = array();
...
if ($objFopen) {
    while (!feof($objFopen)) {
        ...
        if (strlen($word) >= 2 && ctype_alpha($word)) { // Check if word contains 2 or more character and word contains only alphabet
            ...
            $wordFile = fopen("dictionary/{$first_alphabet}/{$first_second_alphabet}/{$word}.txt", "w") or die("Unable to open file!"); // Create word file that contains 100 words
            array_push($structure[$first_alphabet][$first_second_alphabet], $word); // Store word in subfolder array
            for ($iter = 0; $iter < 100; $iter++)
                fwrite($wordFile, $word . "\n");

            // Insert word into Database
            $insert_data = array(
                "word" => $word
            );
            $query = build_insert_query("words", $insert_data);
            $conn->query($query);
        }
    }
    ...
    echo "<h1>File Size (Uncompress) - Report</h1>";
    ...
    // Print output for Compress file
    echo "[STATUS] ".date('Y-m-d H:i:s')." Generate ZIP : START<br>";
    echo "<h1>File Size (Compress) - Report</h1>";
    foreach ($structure as $folder => $subfolder) {
...
$pdf->AddPage('P', 'A4');
$result = $conn->query("SELECT * FROM `words` ORDER BY word ASC");
